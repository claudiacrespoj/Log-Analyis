...
class FileOutputLayer(object):
    def __init__(
        self, log_messages, filename: str, templates: dict, message_headers: list
    ):
    ...
    def outputResult(self):
        ...
        for key in self.templates.keys():
            ...
            log_events[event_tot] = dict(
                EventId=eid, EventTemplate=key, Occurrences=len(self.templates[key])
            )
            event_tot += 1
        ...
        self.output_csv(
            self.filename + "_structured.csv", self.log_messages, self.message_headers
        )
